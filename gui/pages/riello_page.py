...
from core.riello_price import default_price_path, load_price_items, power_modules, rack_cabinets
from core.runtime_paths import resource_path


class RielloPage(QWidget):
    """Страница Riello: выбор ИБП и формирование Excel-расчета по шаблону."""
    ...
    def __init__(self, owner) -> None:
        ...
        owner._add_row(config_grid, 0, "ИБП / шкаф", self.ups_combo, None, None)
        owner._add_row(config_grid, 1, "Кол-во ИБП", self.ups_qty_edit, None, None)
        owner._add_row(config_grid, 2, "Силовой модуль", self.power_module_combo, None, None)
        owner._add_row(config_grid, 3, "Модулей на ИБП", self.modules_per_ups_edit, None, None)
        owner._add_row(config_grid, 4, "Автономия", self.autonomy_edit, None, None)
        owner._add_row(config_grid, 5, "Бат. шкаф", self.battery_cabinet_edit, None, None)
        owner._add_row(config_grid, 6, "Город DDP", self.city_edit, None, None)
        owner._add_row(config_grid, 7, "Курс", self.rate_edit, None, None)
        owner._add_row(config_grid, 8, "Маржа, %", self.margin_edit, None, None)
        owner._add_row(config_grid, 9, "НДС, %", self.vat_edit, None, None)
        owner._add_row(config_grid, 10, "Спецусловие, %", self.special_edit, None, None)
        owner._add_row(config_grid, 11, "Доставка", self.transport_cost_edit, None, None)
        owner._add_row(config_grid, 12, "Таможня", self.customs_edit, None, None)
        owner._add_row(config_grid, 13, "Сертификат", self.certificate_edit, None, None)
        owner._add_row(config_grid, 14, "Дост. клиенту", self.transport_to_customer_edit, None, None)
        owner._add_row(config_grid, 15, "Обследование", self.site_inspection_edit, None, None)
        owner._add_row(config_grid, 16, "Монтаж/ПНР", self.installation_edit, None, None)
        owner._add_row(config_grid, 17, "Доп. расходы", self.extra_cost_edit, None, None)
        ...
        self.hint = QLabel(
            "Для Riello поле «Excel-шаблон» — это файл .xlsx, который будет скопирован и заполнен. "
            "Word-шаблон на этом этапе не требуется. Прайс берется из папки prices приложения."
        )
    ...
    def _load_price_items(self) -> None:
        ...
        try:
            ...
            cabinets = rack_cabinets(self.price_items)
            if not cabinets:
                cabinets = self.price_items[:]
            saved_ups = self._saved("riello/ups_model", "SRT 60 PWC")
            self.ups_combo.clear()
            for item in cabinets:
                self.ups_combo.addItem(item.display_name, item.model)
            ups_index = self.ups_combo.findData(saved_ups)
            if ups_index < 0:
                ups_index = self.ups_combo.findText(saved_ups)
            if ups_index >= 0:
                self.ups_combo.setCurrentIndex(ups_index)
            self._reload_power_modules()
        finally:
            self._updating = False

    def _reload_power_modules(self) -> None:
        ...
        for item in modules:
            self.power_module_combo.addItem(item.display_name, item.model)
        module_index = self.power_module_combo.findData(saved_module)
        if module_index < 0:
            module_index = self.power_module_combo.findText(saved_module)
        if module_index >= 0:
