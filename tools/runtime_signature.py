...
# The app now ships editable Python packages next to the EXE and app.py puts
# that folder first on sys.path.  Because of that, changes in brands/, core/,
# gui/ and other application source files are delivered by App-No-Runtime.zip
# and must NOT change versions.runtime.  Runtime should change only when the
# heavy runtime/dependency layer changes.
IGNORED_TOP_LEVEL_DIRS = {"assets", "brands", "config", "core", "gui", "prices", "templates", "tools"}
IGNORED_FILES = {"app.py", "release_info.json"}
...
def effective_runtime_version() -> str:
    ...
    components = [
        ...
        f"pyinstaller={_package_version('pyinstaller')}",
        f"pyside6={_package_version('PySide6')}",
        f"openpyxl={_package_version('openpyxl')}",
        f"python-docx={_package_version('python-docx')}",
